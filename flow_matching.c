/*
 * Flow Matching generative model from scratch in pure C.
 * Trains on MNIST images (28x28 grayscale) and writes generated samples as PNG.
 *
 * Unlike DDPM (diffusion.c), Flow Matching uses:
 *   - Linear interpolation: x_t = (1-t)*x_0 + t*eps  (no noise schedule)
 *   - x0 prediction:        model predicts clean x_0 from noisy x_t
 *   - Euler ODE sampling:   velocity derived as v = (x_t - pred_x0) / t
 *   - 100 Euler steps (vs 1000 DDPM steps)
 *
...
/* ── Model hyperparameters (same MLP as DDPM) ───────────────────── */
#define T_EMBD 64
...
/* ================================================================
   Sampling (Euler ODE from t=1 to t=0, velocity derived from x0 pred)
   ================================================================ */

static void flow_sample_to_buf(float *out) {
  ...
  float dt = 1.0f / (float)EULER_STEPS;

  for (int step = 0; step < EULER_STEPS; step++) {
    float t = 1.0f - (float)step * dt;
    if (t < 1e-4f) t = 1e-4f;

    timestep_embedding(t * 1000.0f, t_emb);
    ...
    float inv_t = 1.0f / t;
    for (int i = 0; i < IMG_SIZE; i++)
      out[i] -= dt * (out[i] - pred_x0[i]) * inv_t;
  }
}

static void flow_sample_one(int sample_idx) {
  float x[IMG_SIZE];
  flow_sample_to_buf(x);

  char path[256];
  snprintf(path, sizeof(path), "output/flow/sample_%02d.png", sample_idx + 1);
  write_png(path, x);
  printf("Wrote %s\n", path);
}
...
static void free_all(void) {
  ...
  free(dw2); free(db2);
  free(dw3); free(db3);
  free(mw1); free(vw1); free(mb1); free(vb1);
  free(mw2); free(vw2); free(mb2); free(vb2);
  free(mw3); free(vw3); free(mb3); free(vb3);
}
...
int main(int argc, char **argv) {
  int sample_only = 0;
  float lr = 1e-3f;
  const char *outdir = "output/flow";
  const char *weights_path = "flow_matching.bin";
  ...
  if (sample_only) {
    if (!load_weights(weights_path)) {
      fprintf(stderr, "%s not found. Train first with: ./flow_matching\n", weights_path);
      free_all();
      return 1;
    }
  } else {
    ...
    for (int step = 0; step < TRAIN_STEPS; step++) {
      ...
      for (int i = 0; i < IMG_SIZE; i++) {
        eps[i] = rng_gauss(0.0f, 1.0f);
        xt[i] = (1.0f - t) * x0[i] + t * eps[i];
      }
  ...
  for (int i = 0; i < SAMPLE_COUNT; i++) flow_sample_one(i);
